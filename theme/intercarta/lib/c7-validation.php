...
function cpfcnpj_validation_filter( $result, $tag ) {
    $tag = new WPCF7_Shortcode( $tag );

    if ( 'cpf-cnpj' == $tag->name ) {
      $cpf_cnpj = new ValidaCPFCNPJ(isset( $_POST['cpf-cnpj'] ) ? trim( $_POST['cpf-cnpj'] ) : '');

        if ( !$cpf_cnpj->valida() ) {
            $result->invalidate( $tag, "CPF ou CNPJ Inválido." );
        }
    }
...
add_filter('wpcf7_validate_text', 'cpfcnpj_validation_filter', 20, 2 );
add_filter('wpcf7_validate_text*', 'cpfcnpj_validation_filter', 20, 2 );


function wpmidia_activate_masked_input(){
   if( is_page_template('template-budget.php') ){ //mais uma vez, feito isso, o script abaixo só aparecerá na página Contato.
?>

   <script type="text/javascript">
    jQuery("#cpf-cnpj").keydown(function(e){
      try {
        jQuery("#cpf-cnpj").unmask();
      } catch (e) {}

      var tamanho = jQuery("#cpf-cnpj").val().length;
      var code = e.keyCode || e.which;
      if(tamanho < 11){
          jQuery("#cpf-cnpj").mask("999.999.999-99");
      } else if(tamanho >= 11){

        if(code == 9 && tamanho == 11){
          jQuery("#cpf-cnpj").mask("999.999.999-99");
        } else if (tamanho >= 11) {
          jQuery("#cpf-cnpj").mask("99.999.999/9999-99");
        }

      }
    });
    var NinthDigit = function (val) {
    ...
    jQuery(function() {
      jQuery('.phone-cel').mask(NinthDigit, ninthOptions);
...
add_action('wp_footer', 'wpmidia_activate_masked_input', 200);
